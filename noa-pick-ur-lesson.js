...
const showSearchBtn = document.querySelector("#menu_bar01");
const targetNode = document.getElementById("js-container");

//chosen lessons
const storedString0 = GM_getValue("chosenLessons", null);
...
const storedString = GM_getValue("selectedValues", null);
...
function createBtnsNode() {
  ...
  saveBtn.addEventListener("click", () => {
    ...
    locationCheckboxes.forEach((checkbox) => {
      if (checkbox.checked && !selectedValues.locations.includes(checkbox.id)) {
    ...
    GM_setValue("selectedValues", jsonSelectedValues);

    //chosenLessons
    const jsonChosenLessons = JSON.stringify(chosenLessons);
    console.log(jsonChosenLessons);
    GM_setValue("chosenLessons", jsonChosenLessons);
    console.log("saved");
  });
}
...
function initOriginSearch() {
  ...
  if (selectedValues.locations.length > 0) {
    locationCheckboxes.forEach((checkbox) => {
      if (selectedValues.locations.includes(checkbox.id) && !checkbox.checked) {
        checkbox.click();
      }
    });
    locationInit = true;
